...
class MasteryPageAPI(RiotAPIService):
    ...
    def get_mastery_pages(self, query: Mapping[str, Any], context: PipelineContext = None) -> MasteryPagesDto:
    ...
    def get_mastery_pages(self, query: Mapping[str, Any], context: PipelineContext = None) -> Generator[MasteryPagesDto, None, None]:
        if "region" in query and "platform" not in query:
            query["platform"] = Region(query["region"]).platform.value
